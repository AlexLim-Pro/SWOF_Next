const Sketch = (W, H) => (p) => {
	...
	var riverShapeDataFilePath = "/public/assets/NHDFlowline_San_Guad/NHDFlowline_San_Guad.shp";
	...
	var riverConnectivityPath = "/public/assets/rapid_connect_San_Guad.csv";
	...
	var QoutURL = "/public/assets/Qout.csv";
	...
	*/
	...
	function arrayToColor(rgb, alpha = 255) {
		return p.color(rgb[0], rgb[1], rgb[2], alpha);
	}
	...
	function coordsToCanvas(lng = 0, lat = 0) {
		...
		return [(lng - firstX) * xScale * VIEW_SCALE + offsetX,
		(firstY - lat) * yScale * VIEW_SCALE + offsetY];
	}
	...
	*/
	function canvasToCoords(x = 0, y = 0) {
		return [(x - offsetX) / (xScale * VIEW_SCALE) + firstX,
		(y - offsetY) / (yScale * VIEW_SCALE) + firstY];
	}
	...
	function drawEllipses() {
		...
		try {
			coordsHT.forEach((value, key) => {
				...
				// console.log(t);
				let actualCoords = canvasToCoords(p.mouseX, p.mouseY);
...
async function drawLines() {
	...
	*/
	...
	p.setup = () => {
		...
		canvas = p.createCanvas(MAX_LONGITUDE * 2 * xScale, MAX_LATITUDE * 2 * yScale);
		mapBuffer = p.createGraphics(MAX_LONGITUDE * 2 * xScale / 2, MAX_LATITUDE * 2 * yScale);
		rightBuffer = p.createGraphics(MAX_LONGITUDE * 2 * xScale / 2, MAX_LATITUDE * 2 * yScale);
		p.createCanvas(W, H)
	...
	p.draw = () => {
		...
		mapBuffer.stroke(defaultPointColor);
		let actualCoords = canvasToCoords(p.mouseX, p.mouseY);
		mapBuffer.text(`(${actualCoords[0].toFixed(5)}, ${actualCoords[1].toFixed(5)})`, p.mouseX, p.mouseY);
		p.image(mapBuffer, 0, 0);
		if(p.mouseIsPressed) {
			if(mouseDown) {  // dragging mouse
				let endTime = new Date();
				if(endTime - startTime > 200) {
					offsetX += p.mouseX - offsetX;
					offsetY += p.mouseY - offsetY;
				}
			}
			else {
				mouseDown = true;
